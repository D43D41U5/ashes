...
import { POI_TYPES, verifierPlan, verifierVignette } from '../packages/sim/src/index'

const RACINE = resolve(dirname(fileURLToPath(import.meta.url)), '..')
const DOSSIER = resolve(RACINE, 'packages/sim/src/plans')
// LES VIGNETTES DES GROTTES (spec `grottes.md` G-R6) — même grammaire, autre garde, autre registre.
const VIGNETTES = resolve(DOSSIER, 'vignettes')
...
/** Lit et VÉRIFIE un `.plan`, puis l'émet en bloc TypeScript — la même forme pour les deux registres. */
function compiler(dossier: string, fichier: string, garde: (nom: string, plan: Plan) => string[]): string {
  ...
  if (plan.ancre) lignes.push(`    ancre: ${chaine(plan.ancre)},`)
  lignes.push('    grille: [')
...
const blocs = fichiers.map((f) => compiler(DOSSIER, f, (kind, plan) => verifierPlan(kind, plan, POI_TYPES.find((t) => t.slug === kind)?.footprint)))
const vignettes = readdirSync(VIGNETTES).filter((f) => f.endsWith('.plan')).sort()
if (vignettes.length === 0) throw new Error(`aucune vignette dans ${VIGNETTES}`)
const blocsVignettes = vignettes.map((f) => compiler(VIGNETTES, f, verifierVignette))
...
console.log(`${fichiers.length} plans + ${vignettes.length} vignettes → ${SORTIE.replace(RACINE + '/', '')}`)
