...
const SCENARIOS = {
  ...
  /**
   * LES NÉNUPHARS (eau-fond R9) — en GRAPPES, sur l'eau qui ne bouge pas.
   *
   * Ce qui ne se prouve qu'au navigateur : qu'ils naissent (leur lieu est étroit — eau
   * dormante, loin de la rive : ni la rivière, ni la frange d'écume), qu'ils naissent
   * GROUPÉS (une colonie de rhizome, pas des solitaires semés), et que le dessin tient
   * la DA (silhouette d'un seul ton + normale non plate — le juge des `_lit`). On
   * cherche un lac par la carte (bloc 9×9 tout en eau : le lit de la rivière ne fait
   * que 7 tuiles de large, il ne peut pas passer), on s'y téléporte, on regarde.
   * Exige `--dev` (TP).
   */
  async nenuphars(page) {
    await page.goto(URL)
    await page.waitForFunction(() => Boolean(window.__BRAISES__?.scene?.registry?.get('worldReady')), null, { timeout: 150000 })
    await page.waitForTimeout(1000)
    ...
    const lac = await page.evaluate(() => {
      ...
      // Un bloc 9×9 TOUT en eau : plus large que le lit de la rivière (7) → c'est un lac.
      for (let y = 8; y < m.height - 8; y += 2) {
        for (let x = 8; x < m.width - 8; x += 2) {
          if (!eau(x, y)) continue
          let plein = true
          for (let dy = -4; dy <= 4 && plein; dy++) for (let dx = -4; dx <= 4; dx++) if (!eau(x + dx, y + dy)) { plein = false; break }
          if (plein) return { x: x + 0.5, y: y + 0.5 }
        }
      }
      return null
    })
    if (!lac) { console.error('!! aucun lac (bloc 9×9 d’eau) sur cette carte'); return }
    console.log(`lac trouvé @(${lac.x}, ${lac.y})`)

    await page.evaluate(({ px, py }) => window.__BRAISES__.scene.sendAction({ type: 'debug_teleport', x: px, y: py }), { px: lac.x, py: lac.y })
    await page.waitForTimeout(9000) // les colonies naissent toutes les ~1,6 s

    const vie = await page.evaluate(() => {
      const n = window.__BRAISES__.scene.nenuphars
      return { vivants: n?.vivants ?? -1, grappe: n?.plusGrandeGrappe ?? -1 }
    })
    console.log(`nénuphars : ${vie.vivants} feuilles · plus grande grappe ${vie.grappe} ${vie.grappe >= 3 ? '✓' : '✗ (isolés)'}`)
    if (vie.vivants < 3) console.error('!! aucune colonie de nénuphars sur un lac en 9 s')

    // LE DESSIN : un seul ton dans la silhouette (la DA de la masse pâteuse) et une
    // normale qui a du RELIEF (une normale plate tombe en blob bleu à la nuit).
    const art = await page.evaluate(() => {
      ...
      const out = []
      for (let k = 0; k < 3; k++) {
        const tex = sc.textures.get(`fx-nenuphar-${k}_lit`)
        const alb = pixels(tex?.getSourceImage())
        // La normale est le dataSource de la texture _lit (registerLit), pas une clé à
        // part — et c'est une TextureSource : l'image vit dans `.image` (cf. lieux-lit).
        const nrm = pixels(tex && tex.dataSource && tex.dataSource[0] && tex.dataSource[0].image)
        if (!alb) continue
        // Les tons se comptent sur les pixels PLEINS (alpha 255) : le bord antialiasé
        // et le sinus creusé en destination-out rendent des verts à l'arrondi près —
        // ce ne sont pas des matériaux, et les compter faisait crier la garde à tort.
        const tons = new Set()
        let pleins = 0
        for (let i = 0; i < alb.length; i += 4) {
          if (alb[i + 3] !== 255) continue
          pleins++
          tons.add((alb[i] << 16) | (alb[i + 1] << 8) | alb[i + 2])
        }
        // LA PLAQUE (eau-fond R9) : un nénuphar flotte À PLAT — sa normale doit pointer
        // au ciel (nz ≈ 255) sur l'essentiel de sa surface, sinon le soleil l'ombre
        // comme une butte au lieu de l'éclairer d'un seul tenant.
        let nx0 = 255, nx1 = 0, sommeNz = 0, sil = 0
        if (nrm) for (let i = 0; i < nrm.length; i += 4) {
          if (alb[i + 3] < 128) continue
          sil++
          sommeNz += (nrm[i + 2] / 255) * 2 - 1 // décodé : 1 = plein ciel
          nx0 = Math.min(nx0, nrm[i]); nx1 = Math.max(nx1, nrm[i])
        }
        out.push({ k, pleins, tons: tons.size, bord: nrm ? nx1 - nx0 : -1, ciel: sil ? Math.round((sommeNz / sil) * 100) : -1 })
      }
      return out
    })
    for (const a of art) {
      // `ciel` = nz moyen sur la silhouette : 100 % = la face regarde le ciel d'un bout à
      // l'autre (une plaque qui flotte) ; bas = ombrée comme une butte. `bord` = l'étendue
      // de nx, le bourrelet — à zéro, l'arête ne se lit plus du tout.
      console.log(`  gabarit ${a.k} : ${a.pleins} px · ${a.tons} ton(s) ${a.tons === 1 ? '✓' : '✗ (la DA veut UN ton à plat)'} · face au ciel ${a.ciel} % ${a.ciel >= 90 ? '✓' : '✗ (ombrée comme une butte)'} · bourrelet ${a.bord} ${a.bord > 20 ? '✓' : '✗ (arête invisible)'}`)
    }

    await page.screenshot({ path: `${OUT}/nenuphars-lac.png` })
    // Et de nuit : la normale doit sculpter (le juge des `_lit`).
    await page.evaluate(() => window.__BRAISES__.scene.sendAction({ type: 'debug_set_hour', hour: 22 }))
    await page.waitForTimeout(1500)
    await page.screenshot({ path: `${OUT}/nenuphars-nuit.png` })
    console.log(`captures → ${OUT}/nenuphars-*.png — à REGARDER`)
    return { vie, art }
  },
