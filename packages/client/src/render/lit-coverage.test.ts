...
describe('la couverture _lit (garde A1)', () => {
  ...
  const SANS_BASCULE: Record<string, string> = {
    ...
    // (LA NASSE a quitté cette table le 2026-09-13, dans le lot qui l'a rendue atteignable :
    // une recette la produit désormais, donc `snapshot-view` demande bel et bien `st-fish_trap`
    // et le damier magenta était redevenu atteignable. Son art vit dans `bati-art`. C'était
    // écrit ici — « le jour où une recette la produit, elle aura besoin de son art AVANT » —
    // et la garde ② l'a exigé d'elle-même dès que la pièce est entrée dans `BATI_LIT_TYPES`.)
