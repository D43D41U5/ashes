...
describe('le menu du marteau', () => {
  it('R20 : les pièces sont les BARRIÈRES structurelles, pas les composants', () => {
    // Mur, porte, sol, toit, coffre — et RIEN d'actif (enclume, four…) : ceux-là se
    // tiennent et se posent (flux feu de camp), ils n'ont pas leur place ici.
    expect([...BUILDABLES]).toEqual(['wall', 'door', 'floor', 'roof', 'chest'])
  })
  ...
  it('R8 : le matériau change le coût des murs/portes, pas celui des pièces molles', () => {
    ...
    // Sol/toit/coffre : pièces sans palier — le matériau ne les touche pas.
    expect(pieceCost('floor', 'stone')).toEqual(STRUCTURE_COSTS.floor)
    expect(pieceCost('roof', 'metal')).toEqual(STRUCTURE_COSTS.roof)
    expect(pieceCost('chest', 'wood')).toEqual(STRUCTURE_COSTS.chest)
  })
})
