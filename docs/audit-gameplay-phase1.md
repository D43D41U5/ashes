...
18. **Deux stacks de génération de carte** — le banc de calibrage (`pnpm scenario`) tourne sur `valleygen` + `generateNodes`, **une carte que le joueur ne voit jamais**. Tout nombre calibré là est suspect. → `scenario.ts:36` vs `veillee.ts:61` ✅ **RÉSOLU le 2026-08-02** — `scenario.ts` avait migré vers `generateZonedTerrain` ; la pile `valleygen` (5 modules + 2 fichiers de test) est SUPPRIMÉE, il n'y a plus qu'un seul générateur. Voir `docs/decisions.md` 2026-08-02.
