...
- Ce qui manque : **les souillures ne voyagent pas** — ni `protocol.ts` ni `interest.ts` ne les portent (les gouttes, si : même patron à suivre, et `homme` voyage déjà avec elles ; ≤ 64 entrées de quatre nombres, pas de filtre d'intérêt : la traînée s'étend à 40 pas de l'origine). Non fait : un champ de snapshot que rien ne lit serait une loi sans appelant — il vient avec le rendu.
- **La cadence, MESURÉE** (`tools/mesure-recuisson-sang.mts`, monde joué 1581×852, 123 194 tuiles d'eau, fil de 669 pas) : **la recuisson d'aujourd'hui ne peut pas porter le sang.** `regimeDe` interroge `eauSouillee` sur chaque tuile d'eau ; dès qu'UNE souillure de rivière existe, chaque lecture paie `attacheAuFil` (le fil entier) : **1 156 ms pour une souillure, 1 367 ms au plafond de 64**, + 66 ms de `buildWaterField` — contre 17 ms sans sang (la porte O(1), ce que paie la suie une fois par jour). Un gel d'une seconde à chaque recuisson, à n'importe quelle cadence. **La voie exacte et bon marché est prototypée et vérifiée dans l'instrument** : ① une **table d'attache** (le pas de fil de chaque tuile d'eau) cuite UNE fois par le fil et non par la tuile — 6 ms, identique à `attacheAuFil` sur les 123 194 tuiles ; ② à chaque recuisson, chaque souillure **peint son empreinte** (l'aval pas à pas, la tuile n'appartenant qu'à SON pas ; le disque en eau dormante) — **3,8 ms au plafond, identique à `qualiteDeLEau` sur toutes les tuiles d'eau, 2 825 tuiles touchées** ; ③ reste à ne repeindre que le canal B des tuiles touchées (et de celles d'avant) et l'upload de texture — **non mesuré, il se mesure dans le navigateur** (`refresh()` réuploade la texture entière : 1581×852 RGBA ; voir aussi `refresh()` remet LINEAR). Une cadence à la seconde est donc à portée côté CPU ; l'upload décidera.
...
- À poser à Alexis (après l'hypothèse du §1c), une question à la fois : un régime distinct de la suie (canal B : 100 = suie aujourd'hui ; le sang aurait sa valeur et sa teinte dans le shader) ? une teinte qui pâlit avec la force (la loi rend une force dans [0,1] qui décroît sur 5 min et le long de l'aval — le canal B a la place de la porter) ? Puis `da-rendu` pour le rendu, skill `verif-navigateur` pour le voir.
