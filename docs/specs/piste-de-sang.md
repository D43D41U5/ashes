...
*Statut : **LIVRÉ le 2026-09-12** — `pisteStep` dans `faune.ts`, bloc `PISTE` dans `balance.ts`, 22 gardes dans `piste-de-sang.test.ts`. Ce que la livraison a ajouté à la spec est marqué **(livré)** ci-dessous ; une hypothèse y est posée sans Alexis (P1 : le sang de l'homme seulement), signalée comme telle.*
...
- **Le sang de l'homme seulement — hypothèse posée sans Alexis.** La première écriture suivait tout sang (« le sang est le sang ») ; c'est la suite qui a montré ce que ça coûtait (P1, chiffres A26). Le choix est réversible en deux lignes et le tag reste utile dans les deux cas. Variante à considérer si Alexis veut l'histoire du « loup qui vole la prise du chasseur » : suivre aussi le sang d'une bête blessée **par un homme** (la flèche) — un `homme` posé par l'attaquant plutôt que par le saigneur ; non fait.
