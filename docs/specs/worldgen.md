...
>
> **⚑ RÉ-AMENDÉE le 2026-09-20 — LA RACINE JOUÉE EST UN FLANC (spec `flanc.md`).** La hauteur est revenue par étapes : les étages (`etages.md`, 2026-09-01), les terrasses intrazone (`terrasses.md`, 2026-09-03), puis le flanc — une pente sud→nord dans l'uplift (`SOCLE.FLANC_RAPPORT = 2`) et **quatre paliers** (`TERRASSES.PALIERS = 4`) sur le monde joué SEUL. La vallée dormante garde tout ce que dit le bandeau ci-dessus ; « le froid vient du BIOME » est amendé par `flanc.md` F-R6 (le froid monte aussi avec l'étage). Lire les deux bandeaux dans l'ordre.
...
*Source : brainstorm de direction du 2026-07-14 (Alexis), GDD §8bis (les trois cercles), §9 (carte), §13 (roadmap). Statut : **structure implémentée et jouée** (2026-07-14 — graphe, terrasses, seuils, contenu, cendre, ambiance, rendu en marches ; gardes vertes sur la taille de production, A26 retirée avec les buttes — voir R37). **Restent non implémentés** : le partage du brouillard au village (R20), les gardiens par zone (§9 — faune/monstres lisent encore le terrain, pas la zone). *(R19 — le brouillard de guerre — est tenu depuis le 2026-07-24 ; R21 — le seuil s'annonce — est tenu depuis le 2026-07-25 : les BORNES de seuil, dérivées de `map.seuils`, flanquent chaque porte et percent la canopée — brisées sur un secours ; et la frontière de la Cendrière a sa lisière graduée, spec `t0-exploration.md` §4.)* *(Le **froid létal** et le **Névé Blanc**, jadis listés ici comme non faits, sont désormais IMPLÉMENTÉS : `coldDamagePerTick` inflige des dégâts sous `HYPOTHERMIA` et `advanceTemperature` tue (`die … 'cold'`) ; le Névé Blanc est une zone `traverse` à sol de neige praticable (`walkable: true, speedFactor 0.5`). Reste NON tranché : la létalité du froid sur la PLAINE — `ACT_COLD[2]=40` floore pile au seuil `HYPOTHERMIA`, donc la plaine ne tue jamais par le froid, cf. audit Phase 2 §5.)* *(R2 — la paroi dessinée avec son ombre portée — est tenue depuis le 2026-07-14 : `cliff-art.ts` / `cliff-layer.ts`. Le §2bis du même jour la requalifie : arête FINE et marches décalées — la bande de 44 tuiles et l'auto-raccord 3-familles sont à remplacer.)* *La carte est structurée ; elle n'est pas encore DÉFENDUE.* Remplace intégralement la spec worldgen du 2026-07-14 (« la vallée qui pousse à l'exploration »), dont les leçons — et seulement les leçons — sont reportées ici.*
...
- **R32 — TOUTE forme de carte est rectiligne.** Zones, frontières, falaises, seuils, eau, taches de terrain : des rectangles et des polygones à angles droits (axis-aligned). La variété vient de la partition, des proportions et des paliers — jamais des courbes. Le domain warp des frontières et les blobs de bruit sont **abrogés** ; le bruit ne survit que **quantifié au bloc** (une tache de terrain est un pavage de rectangles, pas une éclaboussure).
  > **⚑ AMENDÉE le 2026-08-30 — L'EAU SORT DE R32.** Décision d'Alexis (« je n'ai pas de soucis à dire qu'on arrête les angles droits pour les lacs et rivières »), sur la mesure du canal : deux crops à l'échelle d'un écran montraient la rivière en bande de largeur constante, berges parallèles au cordeau, coins à 90°. **R32 ne gouverne plus l'eau** : elle gouverne encore les zones, les frontières, les falaises, les seuils et le bâti. Tout ce qui COULE (rivière, rus, chenaux) se trace désormais en polyligne méandrée, lissée par Chaikin, ESTAMPÉE AU DISQUE de rayon croissant vers l'aval (`zonegen-trace.ts` — une seule grammaire) ; les **lacs** se peignent à la TUILE, au quantile du champ d'altitude lu en bilinéaire, donc sur une iso-ligne du terrain. Meurent avec : le « coude équerré » et sa garde A2ter (un disque n'a pas de coin extérieur à rater), et l'ancienne définition du coude (`estUnCoude` devient une COURBURE FENÊTRÉE). *La règle de partage devient donc : ce qui pousse est mou, **ce qui coule est mou**, ce qui est taillé est droit.*
  > **⚑ AMENDÉE le 2026-08-22 — CE QUI POUSSE EST MOU, CE QUI EST TAILLÉ EST DROIT.** Décision d'Alexis sur la capture au spawn (« on ne voit que des patchs carrés… pas ouf ») après cinq variations rendues (`docs/decisions.md`). R32 ne gouverne plus que ce qui est TAILLÉ : zones, frontières, falaises, seuils, eau (cuvettes en union de motifs, polylignes orthogonales), bâti. Les **taches de végétation** (ce qui POUSSE : bosquet, fleuraie, lande, sous-bois) se décident **à la tuile**, par lecture interpolée du champ d'humidité (`racine-relief.ts`, grille de 8 inchangée) plus un bruit fin, seuils par quantiles inchangés — et le client les DESSINE en pavés (autotile : frange, liseré, ombre portée), spec `sol-dessine.md` à venir. Mesuré avant : 1 408 bords de tache sur 1 408 tombaient sur un multiple de 8, et un motif fait 40 % de la hauteur d'écran.
...
- **R45 — LES PRÉS BAS PORTENT DE L'EAU, et jamais un mur qui enclave.** La Racine reçoit des **lacs, plans d'eau, une rivière et des ruisseaux** (`zonegen-water.ts`, passe 1.5) — la ligne « prés, bosquets, ruisseaux » du tableau devient vraie. Rectiligne comme le reste (R32) : un cours d'eau est une **polyligne orthogonale** (marches Manhattan) ; pas de berge bruitée ni de méandre (l'organique de `valleygen-water.ts` est abrogé pour la carte jouée).

  > **⚑ AMENDÉE le 2026-07-29 — LE LAC N'EST PLUS UN RECTANGLE, C'EST UNE CUVETTE INONDÉE.** Décision d'Alexis sur la carte rendue (*« on a juste posé l'eau sur le patchwork de biome »*), mise en œuvre par le **micro-relief muet** de `racine-relief.ts` (voir `t0-exploration.md` §2bis et `docs/decisions.md`). Un lac se creuse désormais au point le plus BAS encore libre, sous une lame d'eau qu'on pose et qui inonde ce qui est dessous : sa forme est l'union des motifs de la cuvette — **un polygone rectiligne, jamais un rectangle**. R32 tient (tout est quantifié au motif de 8), et le carré parfait disparaît sans qu'on ait à le lisser. Le cœur profond n'est plus « le rectangle rétréci de trois » mais un **érodé depuis la rive** (BFS au niveau de la tuile), ce qui fait tenir l'invariant de l'anneau sur une forme quelconque. Le reste de R45 est inchangé.

  > **⚑ AMENDÉE le 2026-08-30 — LE CHAMP QUE L'EAU LIT EST DÉSORMAIS ÉRODÉ, ET LE PAYS EST ENDORÉIQUE.** Décision d'Alexis sur l'A/B rendu (« je veux que tu fasses tout pour que j'aie le même rendu », la mer intérieure acceptée : « la mer me va bien »). Le socle savait éroder — mais la Racine y était le NIVEAU DE BASE, donc épinglée, et comme le monde joué EST la Racine, **100 % des tuiles marchables reposaient sur une cellule jamais érodée** (MESURÉ, 3 graines, taille de production). Deux clauses : ① la Racine s'érode (son champ de juillet devient l'uplift maintenu ; le résultat se recale sur l'étendue historique — la forme change, jamais l'échelle, sans quoi la lame de lac deviendrait un plafond) ; ② **aucune cellule n'est niveau de base** — le routage se fait sur la surface nue, chaque minimum local est un terminus, et c'est lui que l'hydrologie inonde. *Un pays qui draine vers un bord n'a pas de lac : il a des rivières qui s'en vont.* Le prix, mesuré et assumé : le plus grand plan d'eau passe de 50 111 à 92 640 tuiles (graine 2026) — une mer intérieure, et l'eau profonde est un mur. Le gain, mesuré : marchable à plus de 40 tuiles d'une eau 30-34 % → 20-24 %, cours d'eau 17-18 → 21-27 par carte. Voir `socle.ts` et `docs/decisions.md`.

  > **⚑ COMPLÉTÉE le 2026-09-12 — LE DÉBIT SURVIT À LA GÉNÉRATION (`map.debit`, reprise de l'eau A1 ; décision d'Alexis : « persister, par tuile »).** L'hydrologie calculait un débit par cellule (`flux`, `rayonDe` → un lit de `RAYON_MIN` 1,2 à `RAYON_MAX` 4,6) puis le jetait : rien au runtime ne savait la taille d'une eau. Désormais un rang 0-7 par tuile, **donnée statique** hachée par `carte-immuable` comme `distEau` et `natureEau` : 0 = terre ou eau DORMANTE (lac, mare, eaux des zones), 1..7 = eau qui COULE — `rangDeDebit`, l'inverse de `rayonDe` (`RAYON_MIN` → 1, `RAYON_MAX` → 7), sur le rayon LOCAL que `peindreCoursDEau` estampe (le rasteriseur le tend au poseur) ; un affluent ne repeint pas le tronc où il se jette, la tuile garde le rang du plus gros. MESURÉ (graine 2026) : 2,69 Mo de JSON (+11 % de carte), 40 406 tuiles d'eau qui coule, le plus gros fleuve va de 1,70 en tête à 7,00 à la bouche ; le rang 1 (un filet sous `TETE_FLUX_MIN`) n'est jamais peint. Aucun lecteur le jour de sa naissance — le « geste zéro » de `docs/reprise-eau.md` : ce qui devinait la taille de l'eau (B2, C2) la lira ici.

  > **⚑ COMPLÉTÉE le 2026-08-30 — LA CAPILLARITÉ (`zonegen-rus.ts`).** Les lacs et la rivière ne suffisaient pas : MESURÉ sur trois graines du monde joué, **80 % de la terre marchable de la Racine était à plus de 40 tuiles de toute eau** — or 40 est `FAUNA.GROUND_WATER_NEAR`, la portée qu'un coin de chasse exige pour naître. La ligne du tableau (« prés, bosquets, RUISSEAUX, lumière ») n'était vraie qu'entre deux lacs voisins. La Racine reçoit donc son **drainage** : priority-flood sur son propre champ, récepteurs D8, accumulation de flux — le patron du socle, avec un autre niveau de base (**les eaux déjà posées** et le bord du pays : les rus descendent DANS ce qui existe). Ils partent des **têtes de vallon** (le drainage y suffit tout juste à creuser) et leur rayon croît avec le flux ; les confluences émergent de l'arbre, on ne les dessine pas. ⚠ Le mécanisme de `zonegen-eaux-zones` était inapplicable ici, et pas par oubli : **la Racine est le niveau de base du socle**, ses cellules n'ont aucun récepteur. Après : 40-42 % au-delà de 40 tuiles, 0,3-1,1 % au-delà de 120 (contre 42-47 %).
  - **Des cours d'eau LOGIQUES.** Un ruisseau RELIE deux plans d'eau — il sort d'un lac et se jette dans un autre. On ne sème pas de moignons qui partent de l'herbe pour finir dans l'herbe. Un lac isolé n'a pas de cours d'eau.

    > **⚑ AMENDÉE le 2026-07-29 — UN RUISSEAU DESCEND ; il ne relie pas le plus proche.** Le « graphe du plus proche voisin entre lacs » est **abrogé** : il joignait deux cuvettes en traversant le dos qui les sépare, ce qui ne se voyait pas tant que les lacs étaient jetés au hasard (sans haut ni bas, aucune liaison ne peut avoir l'air fausse). Un lac s'écoule désormais par son **déversoir** — le point le plus bas de son pourtour — en suivant le **col le plus bas** jusqu'à une eau (plus-court-chemin de GOULOT : le coût d'un chemin est l'altitude maximale qu'il faut franchir, pas sa longueur). Le réseau obtenu est une forêt de drainage : chaque chemin descend, aucun cycle n'est possible. Et le titre de **rivière** appartient à celle qui TRAVERSE (`t0-exploration.md` R5), plus à « la plus longue liaison ». *Un plus court chemin n'est pas un cours d'eau — c'est un canal.*
  - **L'INVARIANT qui rend la connexité gratuite : jamais d'eau profonde sans un anneau de haut-fond marchable la séparant de la terre.** L'eau profonde est un mur (R5), le haut-fond un gué ; seuls les LACS ont un cœur profond, toujours ceint de leur haut-fond ; rivière et ruisseaux sont en haut-fond pur. On contourne donc toujours l'eau à pied — aucune poche n'est enclavée, `garantirLaConnexite` n'a rien à réparer (gardes A2/A5/A12 vertes).
  - **Une frange de MARAIS**, avec parcimonie, ceint toute l'eau (`TERRAIN_MARSH`, gate de bruit quantifié au motif) — la zone basse suinte. `reed_marsh` serait compté comme de l'eau par la faune : on prend le marais franc. Le marais est un **LIT DE ROSEAUX DENSE** (`clutter.ts`, `BIOME_CLUTTER[MARSH]` : density 0.95, understory, roseau dominant) — une roselière, pas un saupoudrage. Et **très rarement une flaque d'eau libre** y perce (`EAU.MARAIS_FLAQUE`, gate par tuile) : du haut-fond, sans incidence sur la connexité. La flaque fait **2×2** et non une case seule — le champ d'eau du shader est filtré linéairement, et l'iso-contour d'un texel isolé est un LOSANGE (carré à 45°) ; un petit carré, lui, se rend proprement. Côté rendu, **le décor de berge (fleurs, touffes) est retiré des tuiles de terre au CONTACT de l'eau** (`clutter.ts`, `touchesWater`) — sinon le débord du bord de l'eau + le décalage du prop y plantent une fleur « dans l'eau ». Les roseaux (REEDY) sont exemptés : ils ont le droit d'y tremper.
  - **Le FOND est marron, MAIS l'eau reflète le CIEL** (feel, `water-layer.ts`). Deux étages : sous la surface, la vase brune du fond (visible sur le gué, elle cède à une eau trouble en profondeur) ; sur la surface, une réflexion du ciel d'autant plus forte que l'eau est profonde. C'est la réflexion qui empêche le plan d'eau de virer au marron sombre uniforme — le fond reste brun là où on le voit, le large prend la lumière du ciel (bleu pâle de jour, éteint la nuit).
  - Le nombre de pièces d'eau est **proportionnel à la surface de la Racine** ; les densités (`EAU` dans `zonegen-water.ts`) sont des ordres de grandeur calibrés au playtest. L'eau reste réservée à la zone basse et vivante : les hauteurs n'ont pas d'eau liquide.
...
- **R17bis — Un emplacement est un site TENABLE, pas seulement bâtissable** *(décision d'Alexis 2026-08-18 — question ③ du rapport de calibration : naître dans un site condamné est un DÉFAUT DE PLACEMENT, pas un sort. R17 est INTACT : cette règle filtre la liste des emplacements — villages PNJ et points de spawn —, `found_village` ne la consulte jamais, le joueur fonde toujours où il veut).* Trois gardes, chacune épinglée à une mort mesurée au banc de saison :
  - **Le territoire de chasse.** Aucun emplacement à moins de `GROUND_RADIUS + SPAWN_RING_MAX` tuiles du centre d'un coin de chasse — la somme est DÉRIVÉE des deux constantes de la faune, jamais écrite : c'est très exactement la portée à laquelle la présence des villageois (les PNJ sont des hôtes d'anneau comme les joueurs) peut faire naître la faune du coin *(amendé 2026-08-28 — faune R28 : plus de prédateurs dans l'anneau ; la meute de loups vit à la Louvière, couverte par l'écart des nids ci-dessous)*. *Mesuré (banc, graine 1234, chasse@76) : 4 fondateurs du Foyer saignés à mort au JOUR 1 après un combat contre la meute de loups née de l'anneau ; graine 2026 (chasse@56) : le Foyer broyé en acte I quand les Braises (chasse@301, tanière@44 !) tiennent à effectif plein.*
  - **Le nid à monstre résident.** Aucun emplacement à moins d'`ECART_NID` tuiles (distance au RECTANGLE de l'empreinte) d'un lieu dont le type porte `monster` (tanière, repaire) — l'ensemble est DÉRIVÉ de `POI_TYPES`, jamais une liste de slugs à la main : un type de nid ajouté demain est couvert d'office.
  - **Le plancher de baies.** La maille de fondation (la même maille+zone que `BOIS_MIN`/`PIERRE_MIN`) porte ≥ `BAIES_MIN` buissons : un village PNJ n'a QU'UNE source de nourriture — les baies ; il ne chasse pas. *(Post-R12β/R15 la famine de placement est quasi éteinte — cette garde est le bon sens qui empêche le désert, pas un sauvetage.)*
- **R18 — Le spawn est ÉPARPILLÉ dans la racine** (décision d'Alexis : *« pour éviter la guerre au lancement »*). N points de départ, tous marchables, tous distants les uns des autres.
  - **R18bis — Le premier point se TIRE, et jamais sur une île** *(décision d'Alexis, 2026-08-31 : « un joueur ne peut pas spawn sur une île isolée en début de partie. On choisit aléatoirement un spawn sur la map mais sur la landmass principale »)*.
    - **La landmass principale.** Un point de naissance appartient à la plus grande composante 4-connexe du marchable (`walkableComponents`), ou n'est pas un point de naissance — le filtre passe AVANT l'écart de fosse. `emplacementsDeVillage` ne sait dire que *marchable* : un carré dégagé au milieu d'un lac est un site viable à ses yeux, et depuis que l'eau est dérivée (les îles) le monde joué porte 21 à 70 composantes. *Mesuré (2026-08-31, 10 graines) : la principale fait 97,5 à 99,5 % du marchable, et **sur la graine 2026 — la vallée canonique de `?solo` et de tous les smokes — le premier point tombait sur une poche de 1 499 tuiles**, coupée du monde ; 0 à 2 des 17 points hors continent selon la graine. Après : 0 sur 10 graines.*
    - **L'ancre se tire au sort**, sur la graine du monde (flux dédié) : même graine, même point — l'invariant de déterminisme tient, deux parties de même graine s'ouvrent au même endroit. Elle était *le site le plus proche du cœur de la racine* : *mesuré sur 10 graines, le premier spawn tombait toujours dans le mouchoir `x ∈ [720, 920], y ∈ [352, 640]` d'une carte de 1581×852, alors que le vivier couvre `x ∈ [72, 1512]`.* Le semis max-min des N-1 suivants est inchangé.
...
- **R19bis — LE MODE DEBUG LÈVE LE BROUILLARD, EN AFFICHAGE SEUL** (demande d'Alexis, 2026-08-27). R19 est une règle de JEU ; elle empêchait le travail d'AUTEUR — juger la silhouette d'un biome, la forme de la vallée, le semis des lieux — qui demande la carte d'un coup d'œil et non une heure de marche. Sous `P` (DEV seulement) la carte plein écran s'ouvre entière : le CALQUE de brouillard est caché, `fog.vu` n'est **jamais** écrit — un dévoilement en dur serait empaqueté par `saveFog` et brûlerait la découverte de la case pour de bon. Les pastilles inconnues deviennent visibles en teinte sourde mais restent **non cliquables** (le filtre `knownPois` de la fiche tient : cf. `annales.md` R16, et 12 px de rayon sur ~58 lieux mangeraient le clic de téléportation). Le libellé dit d'où vient l'image (« ARPENTÉ : x % · DEBUG : carte entière »). Mort deux fois en production : `import.meta.env.DEV` et `debugOn` jamais armé. **Garde : `pnpm smoke --dev --scenario carte-debug`** — terrain visible dans la boîte 0,1 % → 90,5 %, somme de `fog.vu` inchangée, TP encore vivant.
...
- **R-MR3 — La boucle de saison entière survit dans le plus petit monde possible.** L'impasse donne ses **2 seuils** à l'unique frontière (1 secours), les sentes partent de ces seuils, les gués existent, le front est armé, villages et spawns vivent dans la racine. **Et le front est une marée SUD→NORD** : dans le monde réduit, la distance au front se mesure au RECT de la Cendrière — l'heuristique du « voisin d'en face » ment à deux zones (MESURÉ le 2026-08-18 : champ = 8 au bord nord à 520 tuiles du feu, 37 % de la racine brûlait à contre-sens ; trouvé par la dérivation des carrières de §2sexies, corrigé le jour même).
...
**Gardes** (`monde-reduit.test.ts`) : **A-MR1** le monde réduit tient (2 zones, 2 seuils, sentes, ≥ 2 gués, front armé, ≥ 3 villages et des spawns dans la racine, carte coupée ; + balayage 8 graines à petite échelle) ; **A-MR2** géométrie absolue des rects identique au plan complet (w×h, x, emboîtement vertical) ; **A-MR3** déterminisme au bit près du plan réduit (A12 étendu) ; **A-MR4** le banc joue `MONDE_JOUE` (parité de `construireMondeDuBanc`, quel que soit le plan courant) ; **A-MR5** le front est une marée sud→nord — aucune tuile de racine ne brûle hors de portée du rect de la Cendrière (± le grain du bloc).
...
| **`temperature.ts`** | **Le froid TUE** — dégâts sur la durée sous un seuil (décision d'Alexis). **FAIT** — le froid inflige désormais des dégâts sous `HYPOTHERMIA` (`coldDamagePerTick`, `temperature.ts`) et peut TUER (`advanceTemperature` → `die … 'cold'`). Reste à trancher : la létalité sur la PLAINE (calibrage `ACT_COLD`, cf. audit Phase 2 §5). | Sans ça, Alpages / Névé / Glacier ne sont pas des gates. |
| **`balance.ts` — `TERRAINS`** | **La neige devient praticable** (lente, mortellement froide). **FAIT** — `snow` est désormais `walkable: true, speedFactor: 0.5` (`balance.ts`) : praticable et lente, et le malus de froid s'y applique bel et bien. | Sans ça, le Névé est impossible. |
...
| **A17bis** | **Tout emplacement est TENABLE** (R17bis) : aucun à moins de `GROUND_RADIUS + SPAWN_RING_MAX` d'un coin de chasse, aucun à moins d'`ECART_NID` du rectangle d'un lieu à monstre résident, tous avec ≥ `BAIES_MIN` baies dans leur maille — garde EXHAUSTIVE sur la liste rendue, toute seed de garde, ET A17 reste vert (le filtre ne vide pas la racine). |
| **A18** | **Le spawn est éparpillé** : N points de départ dans la racine, tous marchables, tous mutuellement distants (R18). |
| **A18bis** | **On ne naît jamais sur une île, et l'ancre se tire** (R18bis) — sur le MONDE JOUÉ (`monde-reduit.test.ts`, seeds de production) : ① les N points appartiennent tous à la plus grande composante marchable, la carte portant bien plusieurs composantes (la prémisse est affirmée, sinon la garde ne prouve rien) ; ② le premier point n'est plus le plus proche du cœur de la racine — affirmé **sur les points RENDUS** (reconstituer le vivier filtré au-dehors oublie un filtre : la garde passait au vert contre l'ancienne règle) et **sur l'ENSEMBLE des graines** (un tirage honnête sort l'ancre du cœur une fois sur N) ; deux graines donnent deux ancres, et deux appels de même graine le même semis au point près. |
