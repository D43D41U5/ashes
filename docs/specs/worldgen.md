...
- **R45 — LES PRÉS BAS PORTENT DE L'EAU, et jamais un mur qui enclave.** La Racine reçoit des **lacs, plans d'eau, une rivière et des ruisseaux** (`zonegen-water.ts`, passe 1.5) — la ligne « prés, bosquets, ruisseaux » du tableau devient vraie. Rectiligne comme le reste (R32) : un lac est un **rectangle** aligné au motif, un cours d'eau une **polyligne orthogonale** (marches Manhattan) ; pas de berge bruitée ni de méandre (l'organique de `valleygen-water.ts` est abrogé pour la carte jouée).
  - **Des cours d'eau LOGIQUES.** Un ruisseau RELIE deux plans d'eau — il sort d'un lac et se jette dans un autre. On ne sème pas de moignons qui partent de l'herbe pour finir dans l'herbe. Le réseau est le graphe du plus proche voisin entre lacs (dédupliqué) ; sa plus longue liaison est la **rivière** (plus large). Un lac isolé n'a pas de cours d'eau.
