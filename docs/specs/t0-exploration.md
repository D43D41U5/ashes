...
- **R5 — LA rivière TRAVERSE la Racine du nord au sud.** Elle naît au pied d'une frontière de la ceinture (l'eau descend des hauteurs), enfile 1 à 3 lacs, et meurt à la frontière de la Cendrière : **l'eau descend vers le feu**, et le joueur qui la suit apprend la géographie de la saison. ~~Tracé Manhattan quantifié au motif (R32), comme tout le reste.~~ **Tracé COURBE depuis le 2026-08-30 — voir R5bis.**
...
- **R5bis — ⚑ LE TRACÉ N'EST PLUS MANHATTAN (2026-08-30).** Décision d'Alexis (« arrête les angles droits pour les lacs et rivières ») sur la mesure du canal : deux crops à l'échelle d'un écran, en amont et en aval, montraient la même largeur, des berges parallèles au cordeau et des coins à 90°. La rivière **méandre** (bruit sur la normale, fenêtre en cloche — la source et l'embouchure restent EXACTEMENT où le col les a mises), sa courbe est lissée (Chaikin) puis rastérisée en chaîne 4-connexe (**le contrat du `fil` tient au bit près**), et son lit s'**estampe au disque**, de rayon croissant vers l'aval (2,4 → 4,4 tuiles) plus un battement de berge. Le cœur profond suit, en retrait constant : l'anneau de haut-fond de R45 tient toujours par construction. La rivière **grossit donc en descendant**, et ça se voit — c'est ce qui dit au joueur dans quel sens l'eau va. Voir `worldgen.md` R32 (amendée) et `zonegen-trace.ts`.
- **R6bis — ⛔ CADUQUE LE 2026-08-30** (le défaut qu'elle réparait ne peut plus exister : le lit s'estampe au DISQUE le long d'une courbe, et un disque n'a pas de coin extérieur à rater — la garde A2ter est réécrite en « le lit est CONTINU »). *Ce qu'elle disait :* **Le COUDE est ÉQUERRÉ.** *(ajouté le 2026-07-26, sur constat d'Alexis « l'extérieur des coudes est bizarre ».)* Le lit se peint en bandes perpendiculaires au fil ; au virage, chaque bras s'arrêtait au pivot et le quart EXTÉRIEUR n'appartenait à aucun des deux — un bloc de `demi × (demi+1)` = **12 tuiles sèches plantées dans le coin de chaque coude** (MESURÉ : portée de l'eau sur la diagonale extérieure, médiane **0,00 tuile** contre 4,24 à l'intérieur, là où un coude carré donnerait 4,24 des deux côtés). On pose donc, sur CHAQUE pivot, le **carré plein de côté `2·demi+1`** — ce qui revient exactement à prolonger chaque bras de `demi` tuiles au-delà du virage. La berge extérieure tourne son angle droit au lieu de couper le virage ; le cœur profond s'équerre pareil (carré de côté 3). Même géométrie pour les ruisseaux (`tracerChenal`, qui n'avait aucune correction de coude), à leur échelle. Rectiligne (R32) : un coude reste deux droites et un angle droit, jamais une courbe.
...
> **⚑ AMENDÉE le 2026-08-30 — LE CHAMP N'EST PLUS UN BRUIT, IL A UNE HISTOIRE.** Décision d'Alexis sur l'A/B rendu (« je veux que tu fasses tout pour que j'aie le même rendu que dans l'artefact »), la mer intérieure qui l'accompagne étant acceptée (« la mer me va bien »). Le socle (`socle.ts`) savait éroder — uplift, stream power, drainage D8 — mais la Racine y était le **niveau de base**, donc **épinglée** : comme le monde joué EST la Racine, **100 % des tuiles marchables reposaient sur une cellule jamais érodée** (MESURÉ, 3 graines, taille de production). Le champ d'ordre de R22 restait le fbm de juillet, et l'hydrologie dérivée lisait un bruit. Désormais : **la Racine s'érode** (son champ de juillet devient l'uplift maintenu, l'incision le dissèque, et le résultat se recale sur l'étendue historique — la forme change, jamais l'échelle), et **le pays est ENDORÉIQUE** — aucun niveau de base, chaque minimum local est un terminus où le flux s'accumule et que l'hydrologie inonde. La chaîne de R23 est inchangée dans son ordre ; c'est sa PREMIÈRE variable qui cesse d'être du bruit. Conséquences mesurées : la part de marchable à plus de 40 tuiles d'une eau tombe de 30-34 % à 20-24 %, les cours d'eau passent de 17-18 à 21-27, et le plus grand plan d'eau de 50 111 à 92 640 tuiles (graine 2026) — la mer intérieure, assumée. Voir `docs/decisions.md` et l'en-tête de `socle.ts`.
...
- **R27 — LE LAC EST UNE CUVETTE INONDÉE** (amende `worldgen.md` R45) : point le plus bas, lame d'eau posée, on inonde ce qui est dessous, cœur profond **érodé depuis la rive**. ~~Forme : union de motifs — rectiligne (R32), jamais un rectangle.~~ **⚑ 2026-08-30 : la forme se peint À LA TUILE, au quantile du champ d'altitude lu en bilinéaire (`LAC_REMPLISSAGE` 0,86 de l'emprise) — une iso-ligne du terrain, donc une rive COURBE et dentelée par son grain. Et le plafond d'inondation se TIRE par lac : les cuvettes étaient 8 à 11 sur 17-20 à tenir dans 4 % d'écart autour de 2 100 tuiles — le plafond était saturé, donc c'était LUI qui donnait sa taille à chaque lac.** On inonde la **grande ondulation** seule : à deux octaves, les lacs sortaient filandreux (l'octave fine creuse des rigoles que l'inondation enfile au lieu de remplir la cuvette).
