...
- **R12 — Découplage art↔grille.** `TILE_PX` désigne désormais **uniquement** l'échelle grille↔pixels (le pont), plus « la taille de l'art ». Chaque acteur porte son **emprise en tuiles** et se dessine en **origine pieds (0,5 ; 1)** posée au bas de sa tuile logique, via `setDisplaySize(footprintTiles × TILE_PX)`. Conséquence : remplacer un placeholder par un asset de résolution arbitraire ne change **ni sa taille monde ni son emprise** — aucune math de layout touchée. L'emprise *logique* (collision/clic, `AVATAR_HITBOX_TILES`) est inchangée : c'est le *visuel* qui se découple et peut être plus haut que l'emprise. *(Amendé le 2026-07-09 : les nœuds et les structures sont désormais eux aussi en **origine pieds** — bas-centre de leur tuile, via `tileFeetAnchor` — et non plus ancrés au coin haut-gauche. Une cime ou un toit plus hauts que leur tuile « montent » sans décaler ni leur tri ni leur emprise. Seul le sol garde son ancrage grille.)*
- **R13 — Profondeur par Y (Y-sort), UNE seule bande.** *(Étendu le 2026-07-09 : à l'origine seuls les acteurs et les structures triaient ; les nœuds étaient à `depth 4` et le décor sous 3, donc aucun arbre ne pouvait passer devant le joueur.)* Tout ce qui a des **pieds** — acteurs, structures, nœuds, décor, cadavres — trie dans la même bande, par `ySortDepth(feetY, tilePx, tie) = Y_SORT_BASE + feetY × tilePx + tie`. **Une unité de depth = un pixel monde** : un sprite dont les pieds sont un pixel plus bas passe devant. `tie ∈ [0,1)` ne départage que les égalités exactes (cadavre < décor < nœud < structure < acteur) et ne peut jamais renverser un écart réel. Restent hors bande, à plat : le sol, le foyer d'un Feu, et les props **rampants** (cailloux, lichen, sphaigne). Ce qui distingue un conifère décoratif d'un arbre récoltable est la **teinte** (INV-2), pas la couche.
  - **Le plafond suit la carte.** La bande monte à `Y_SORT_BASE + hauteurCarte × TILE_PX` (≈ 58 600 sur 3600 tuiles). Les couches qui coiffent le monde (canopée, voile de nuit, halos, aperçu de construction) sont donc placées **très** au-dessus (10⁶ et plus) et non à quelques milliers — sans quoi, passé la rangée y ≈ 1000, un acteur se dessinait par-dessus la canopée et la nuit.
  - **Les sprites poolés** (nœuds, décor) recalculent leur depth **à chaque frame**, jamais à la création : un sprite réutilisé sur une autre tuile garderait sinon l'ancienne valeur.
