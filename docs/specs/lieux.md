...
- **A1** — Au tick 0, `knownPois` est vide et la carte plein écran n'affiche **aucune** pastille de lieu. ~~Le terrain, lui, est entier (biomes, relief, routes).~~ **Amendé le 2026-07-14 (R2bis) — mais DIFFÉRÉ, pas encore codé** : la cible est qu'au tick 0 le terrain ne soit PAS entier non plus (seule révélée l'emprise parcourue). **En l'état, le brouillard de guerre du TERRAIN n'existe pas** : le client ne masque que les pastilles de POI via `knownPois` (`UIScene.ts`), le sol reste montré en entier — le comportement A1 d'origine (cf. `worldgen.md` R19-R20, « non implémentés »). Le Belvédère, lui, dévoile du **terrain** en plus des lieux : monter, c'est voir.
...
- **Les familles `eco` / `shelter` / `danger`** gardent leur rôle actuel (gisements, monstres) : cette spec ne les recharge pas. Seule exception : `isSheltered` apprend la Grotte (R10). *(⚠ Réserve 2026-07-19 : sur la carte jouée, le `nodeKind` d'un POI `eco` (`gisement`/`carrière`) est **inerte** — `placeZoneNodes` distribue les nœuds par la table de ZONE, pas par le POI. Un « Gisement » est une étiquette : il ne garantit pas de minerai sous son emprise (roche nue hors Karst). Chantier ouvert, cf. audit Phase 2 R-G.)*
