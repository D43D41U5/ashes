...
*Statut (2026-09-13) : **LIVRÉ — et ATTEIGNABLE EN JEU** (recette, item, art et pose hors village : un second lot le même jour, voir plus bas).* `NASSE` (`balance.ts`), l'entrée `fish_trap` du registre (`pieces.ts`), `eauSeule` (`pieces.ts` + `terrainConstructible`), `advanceNasses` (`nasse.ts`, branchée dans `step()`), l'événement `nasse_caught` (`events.ts`, muet à l'oreille dans `audio/inventaire.ts`) ; `tirerLaTaille` exporté depuis `economy.ts` pour que la nasse tire la taille avec la MÊME loi que la canne. `NASSE.CAPACITE` (le plafond de prise, voir la boucle). **23 tests** (`nasse.test.ts`, N1-N16).
⚠ **ATTEIGNABLE DEPUIS LE 2026-09-13 — et c'est un SECOND lot qui l'a fait.** Le matin, la nasse était un système complet, dérivé et gardé, dont **rien au monde ne pouvait poser un exemplaire** (*une loi livrée sans appelant* : vingt gardes vertes sur un mécanisme que le jeu n'appelait pas). Il lui manquait trois choses, livrées depuis : une **recette** (`RecipeId` `fish_trap` — 8 fibres, `requiert: null`, donc à la main et sans atelier : on la veut dès qu'on a cueilli à la roselière, et **sans palier de corde**, là où le séchoir en demande deux) ; un **`ItemId`** (avec son poids de portage, 3 — de l'osier encombrant mais creux — et son rayon « campement » au panneau de fabrication) ; et son **art** (`bati-art` pour l'ouvrage posé, `item-art` pour l'icône du sac). Plus une **décision d'Alexis** : **`horsVillage: true`** — la nasse se pose sur n'importe quelle rive, **même sans foyer**. Sans ce drapeau, `place_component` la refusait deux fois (« sans village — fonder un foyer d'abord », puis « hors du carré du Feu », rayon 10/13/16 tuiles **autour du foyer**) : elle n'aurait été posable que dans un village dont le carré touche l'eau, et l'axe D3 — « pourquoi aller à la roselière » — mourait de ne pouvoir poser le piège LÀ-BAS. La chaîne entière (fabriquer → tenir → poser) est gardée par **N16**, qui joue les actions réelles plutôt que de balayer les tables, et qui a été éprouvée par FALSIFICATION : drapeau retiré, elle rougit sur « sans village — fonder un foyer d'abord ». ⚠ **LE SPRITE A ÉTÉ VU AU NAVIGATEUR — et il était CASSÉ.** Posée, la nasse sortait en **texture manquante** de Phaser (le carré vert barré) sur **une rive sur deux** : mon `dresse: false` la privait de son retourné `_lit_m`, que `snapshot-view` (l.1892) réclame via un **hash de la tuile** — et c'est le seul appelant du client à passer ce bit NU, parce que la table promet en tête que toutes ses pièces se dressent. Corrigé (`dresse: true`), plus une garde dans `bati-art.test` qui affirme le retourné pour TOUTE la table (les deux autotuilés exceptés, leurs appelants conjuguant le bit avec `masqueSeRetourne`). Trois de mes gardes regardaient à côté : `tsc` et le lint ne voient pas une clé faite par concaténation, et la partition `_lit` n'exige que la clé SANS miroir. **Deux dessins ont été refaits sur ce qu'on a vu** : l'ouvrage disait « pot » et l'icône « caisse » (des masses pleines) ; ce sont maintenant des **arceaux ajourés**, où l'eau se voit entre les brins — c'est elle qui fait lire un corps immergé plutôt qu'une caisse posée dessus (la règle est celle du séchoir : « pas de boîte, le sol se voit entre les barres »). Et **la prémisse de couleur était fausse** : le haut-fond rend **pâle, gris-sable**, pas bleu-vert, donc l'ocre chaud s'y noyait ; comme la nasse se pose sur n'importe quelle rive (`horsVillage`), elle n'a pas UN fond à contrer et porte désormais son contraste **en elle** (rive haute claire, flancs et dessous les plus sombres, goulot noir). Le jugement esthétique final revient à Alexis, captures en main. Reste ouvert aussi : le CALIBRAGE en jouant (cadence, `CAPACITE`, coût, pouvoir des appâts). Hors périmètre v1, dit et tenu : aucune relève par les PNJ.*
