...
- **T9 — La repousse passe de 5 minutes à UN CYCLE**, modulée par l'acte. *(Écrit « 45 minutes » tant que le cycle durait 45 min ; déclaré en cycles depuis le 2026-08-24 — `NODE_REGROW_TICKS: ticksForCycles(1)`. La quantité calibrée est « une journée », pas un nombre de minutes.)* Une clairière qu'on rase reste vide pour la journée : **on va voir ailleurs**, et c'est là que tout commence (GDD §8bis : la collecte met le joueur sur les routes, donc dans les rencontres).
...
- **T11 — LES TROIS CERCLES** (`CIRCLES`) : autour du point de départ, la récolte est **médiocre** (« un village y survit, n'y prospère jamais ») ; la richesse est au loin, avec ce qui y vit. *C'est pourquoi la géographie vient APRÈS le poids : maintenant que s'éloigner coûte, il faut que ça rapporte.* *(⚠ Signalement 2026-07-19 : sur la carte Veillée réellement jouée, `circleFactor` a été retiré (`zone-content.ts`) — la rareté est désormais ZONALE, pas radiale ; seul le gradient de DANGER, `predatorBias` (T11bis), subsiste. Re-corréler richesse et danger est l'axe V2-19.)*
- **T11bis — …et le loin est DANGEREUX** *(amendé 2026-08-28 — le gradient de tirage `predatorBias` est RETIRÉ avec le canal ambiant des prédateurs, faune R28)* : le danger des marges est désormais porté par des **LIEUX** — les **Louvières** (meutes résidentes — `pres_bas`, le T0, seul monde joué ; leurs zones de vallée se redécideront quand elle reviendra) et leurs coins de chasse garantis — et par la **nuit qui chasse**, plus forte aux actes tardifs. Le principe tient : s'éloigner rapporte ET fait peur ; mais le danger s'**apprend et se contourne** au lieu d'être un brouillard de tirage.

### 5. LA NUIT CHASSE

*(⚠ ÉTEINTE DANS LE VRAI JEU « pour l'instant » — décision d'Alexis, 2026-08-28, prise avec la Louvière (faune R28) : l'hôte passe `nightHunt: false` (Veillée `worker/veillee.ts`, LAN `server/scenario.ts` — patron `meteoActive`). Le mécanisme reste écrit, testé (`louviere.test.ts` : l'interrupteur et son témoin) et armé par défaut dans les bancs ; la rallumer, c'est re-flipper la ligne des deux hôtes.)*
