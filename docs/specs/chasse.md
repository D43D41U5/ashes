...
- **C12 — LE SANG APPELLE LES LOUPS.** La tuyauterie de la satiété (R15) existe ; on lui donne un odorat :
  ...
  - *(Amendé 2026-08-28 — faune R28 : le biais de SPAWN par le sang (`BLOOD_PREDATOR_BIAS`) est parti avec le canal ambiant des prédateurs. C12 tient par le **flair des loups vivants** — `CARCASS_SEEK_FRESH`, ci-dessus — et les loups vivent désormais quelque part : à la Louvière, dont le coin de chasse est garanti.)*
...
- **A8 (dégénérescence)** — Tous les tests faune existants (88) passent inchangés ou avec des deltas expliqués un à un : la jauge, réglée aux portées actuelles, reproduit les comportements R5/R6 pour une approche naïve.
