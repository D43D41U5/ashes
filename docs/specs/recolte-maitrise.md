...
- **P4 — Le bonus de rendement passif de `foraging` DEMEURE** (`economy.ts`, `SKILL_YIELD_BONUS`, déjà dans le moteur) : l'expert tire plus de CHAQUE nœud. La perception dit **où**, le bonus dit **combien** — les deux faces de la maîtrise, aucune n'étant un mini-jeu par plante.
