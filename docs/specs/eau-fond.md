# eau-fond — le fond de l'eau, dix gestes proposés, sept vivants

*Source : artefact « Le fond de l'eau — dix gestes » (2026-07-26, état des lieux complet + 10 propositions), direction d'Alexis « on lerp case à case comme les transitions de biomes sur terre » sur la frontière, puis « fais tout », puis la revue d'œil du même soir — trois gestes RETIRÉS (l'eau morte du marais, le marnage, les nénuphars), trois REPRIS (transition fondue puis élargie, stries cassées, herbiers en tuiles). Statut : **livré le 2026-07-26**. Prolonge `eau-vivante.md` — même cadre acté : grain 4 px NEAREST, paliers francs pour les FX, un seul soleil, zéro post-FX, R45, gué ≥ 1,4:1 (R10 da-feeling), le profond est un mur, `/sim` intact (tout ce chantier est client seul — pas un octet de sim touché).*
...
- **R9 — LES HERBIERS** (geste 09, repris). Ils vivent dans le **LANGAGE DU SOL** : des TUILES d'algues dans `uFond` (le premier jet en blobs shader était hors DA — retiré) — massifs ouverts par un hash de région (blocs de 8 tuiles), semés par un hash fin, haut-fond CALME seulement (courant < 0,1), à plus de 1,3 t de la rive (le bord reste sable). *(Les NÉNUPHARS ont accompagné ce geste une soirée puis ont été retirés — voir plus bas.)*
...
- **Les nénuphars** (moitié du geste 09) : retirés le 2026-07-26 après trois passes de reprise toutes mesurées (colonies de rhizome en eau dormante — grappe max 6 ; silhouette elliptique fendue d'un sinus, un seul ton ; `normalePlaque` — une feuille FLOTTE, sa face regarde le ciel de bout en bout : 34 % → 98 % de la surface au ciel). Le module `world/nenuphars.ts` et le scénario smoke `nenuphars` sont supprimés avec. **La leçon reste, elle, réutilisable pour tout futur prop posé à plat sur l'eau : `normalFromCanvas` lisse la silhouette en BUTTE, ce qui est juste pour une masse (buisson, roche, dalle) et faux pour une plaque — il lui faut une normale (0,0,1) écrite à la main, seul le dernier pixel du bord incliné.** Les herbiers du fond, eux, restent.
...
- **Roseaux/plantes d'eau récoltables** : de vrais nœuds /sim, chantier séparé (règle « objets de jeu réels »).
