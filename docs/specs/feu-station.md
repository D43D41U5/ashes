...
- **S30 — LE FEU LAISSE DU CHARBON DE BOIS** (décision d'Alexis, 2026-08-25). Toutes les `FIRE.CHARBON_PAR_BUCHES` (4) bûches **entièrement consumées**, le feu dépose **1 `charcoal`** dans ses SORTIES. C'est un SOUS-PRODUIT au sens de S27 — la case de sortie était déjà décrite comme « résultats + sous-produits » —, mais un sous-produit de la **COMBUSTION** et non d'une recette de cuisson : `recettesDuPoste` ne le connaît pas, `fireZoneAccepts` le laisse donc entrer explicitement (sans quoi un feu refuserait qu'on repose le charbon qu'il vient de produire).
  ...
  - **⚠ CE N'EST PAS `coal`, ET C'EST LA DÉCISION.** La houille se MINE (filon, affleurement charbonneux, butin de convoi) et reste la seule à fondre le fer (`iron_ingot`, `steel_ingot`). Question posée, arbitrée : chaque feu de camp produisant la houille aurait dissous en silence le goulot qui borne la carte à un village équipé (`racine-relief.ts`, `CREUX.AFFL_IDENTITES`). Une garde balaie `RECIPES` et rougit le jour où quelqu'un l'ajoutera à une recette.
  - **⚠ IL N'A PAS ENCORE DE CONSOMMATEUR, ET C'EST DIT.** Il se produit, se porte (0,3 — presque rien, contre 2 pour la houille), s'empile (20) et se stocke ; rien ne le lit. C'est un socle posé pour **la carte au fusain** (relever depuis un point haut, façon *The Long Dark*), qui reste à décider et à construire. On ne lui invente pas un usage pour combler le vide — « le charbon brûle plus longtemps » est l'item différé de S13, et une autre décision.
