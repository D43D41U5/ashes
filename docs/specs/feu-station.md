...
### Le charbon de bois (S30)

- **S30 — LE FEU LAISSE DU CHARBON DE BOIS** (décision d'Alexis, 2026-08-25). Toutes les `FIRE.CHARBON_PAR_BUCHES` (4) bûches **entièrement consumées**, le feu dépose **1 `charcoal`** dans ses SORTIES. C'est un SOUS-PRODUIT au sens de S27 — la case de sortie était déjà décrite comme « résultats + sous-produits » —, mais un sous-produit de la **COMBUSTION** et non d'une recette de cuisson : `recettesDuPoste` ne le connaît pas, `fireZoneAccepts` le laisse donc entrer explicitement (sans quoi un feu refuserait qu'on repose le charbon qu'il vient de produire).
  - **LES DEUX FEUX, UNE SEULE RÈGLE.** Le feu LIBRE compte ses bûches une à une (`structure.buchesConsumees`, +1 à chaque bûche finie). Le FOYER n'a pas de bûches mais un évier continu : il convertit ce qui se consume à `village.fuel` (`FIRE_UPKEEP.FEED_PER_WOOD` de stock = une bûche) dans `village.charbonDette`, que `advanceFire` verse dans le même compteur — là où il tient déjà la structure, sans un balayage de villages par tick.
  - **ON COMPTE LE DELTA CONSOMMÉ, JAMAIS LE DRAIN NOMINAL.** À sec, `advanceUpkeep` soustrait quand même et `Math.max(0, …)` rattrape : une dette prise sur le drain ferait produire du charbon à un Foyer mort, indéfiniment, à partir de rien.
  - **⚠ ASYMÉTRIE VOULUE, ET ELLE SE MESURE.** Sur le Foyer, la dette se prend **avant** la soif des Cendreux (⑯) : boire vide un stock, ça ne brûle rien — sinon un siège serait une fabrique de charbon. Sur le feu LIBRE, l'inverse : la soif et la pluie reculent l'ancre `burnAt`, donc elles font vraiment **finir** des bûches, et une bûche finie est une bûche finie. *(Mesuré au vrai tick : 40 000 ticks avec un Cendreux au Foyer → 7,73 bûches perdues au stock, 3,17 seulement consumées.)*
  - **RIEN NE SE PERD.** Sortie pleine ⇒ la dette **reste due** et se solde à la première case libérée. Même doctrine que A17 pour les cuits.
  - **DÉTERMINISTE ET SANS TIRAGE** : un seuil sur un compteur, pas une chance. Quatre bûches, un charbon, sur tout moteur.
  - **⚠ CE N'EST PAS `coal`, ET ÇA RESTE LA DÉCISION.** La houille se MINE (filon, affleurement charbonneux, butin de convoi). Le risque nommé le 2026-08-25 — *chaque feu de camp produisant la houille aurait dissous en silence le goulot qui borne la carte à un village équipé* — tient toujours, et c'est lui que la garde protège.
  - **⚠ IL A UN CONSOMMATEUR DEPUIS LE 2026-08-27 — et cette ligne disait l'inverse.** Elle disait : *« il n'a pas encore de consommateur, et c'est dit »*, en attendant la carte au fusain. Alexis a tranché autrement (**R24**, `cendre.md`) : **le charbon de bois est le combustible de forge du pauvre** — `iron_ingot_charbon`, deux charbons pour un lingot de fer, à la Forge N2. **L'ACIER RESTE MINIER** (`steel_ingot` n'accepte que la houille), et le rapport 2:1 est le prix : un lingot au charbon coûte huit bûches et le temps de feu. Le goulot ci-dessus n'est donc pas dissous, il est CHIFFRÉ.
    - La garde de ce bloc a été **retournée**, pas supprimée : elle affirmait « aucune recette ne le consomme », elle affirme désormais que toute recette au charbon **ne sort pas d'acier** et **coûte plus d'unités que son jumeau à la houille**. C'est le même garde-fou, sur la forme qu'il a prise.
    - *(« Le charbon brûle plus longtemps » reste l'item différé de S13 ; la carte au fusain reste à décider.)*
  - **Hors de l'encyclopédie** : sa section « ressources » se dérive de `NODE_DEFS` (ce qui se RÉCOLTE), et le charbon de bois ne se récolte pas. Comme les cuits, il n'y a pas de fiche.
