...
- **G-R12 — La paroi tient une tuile (Alexis, 2026-09-20).** Entre l'intérieur d'une grotte et la falaise extérieure — s'il y en a une — il y a toujours au moins `KARST.PAROI` (1) tuile de roche. La masse est ce qui est au palier ≥ `p+1` ; le dehors, tout ce qui est en dessous (la face par où la gueule s'ouvre, et toute autre face de la même terrasse) et le bord de la carte. Une tuile ne se creuse que si ses huit voisines sont dans la masse ; seules les PORTES touchent le dehors : la paire de gueule (au palier `p`, c'est le dehors lui-même) et les deux tuiles juste derrière, le seuil qu'on franchit. Un nœud de boyau ou un front de salle qui ne trouvent pas cette marge ne se creusent pas ; une gueule secondaire dont le seuil ne la tient pas cède ; un karst dont la gueule principale ne peut pas la tenir n'est pas creusé — la roche cède avant le compte, comme R13.
...
Réglages de CARTE, à côté du générateur (`export const KARST = {` en tête de `zonegen-karst.ts`) : `TUILES 300` (borne haute), `GUEULES_MAX 4`, `GUEULES_ECART 8`, `BOYAU_LARGEUR 2`, `VESTIBULE_RANGEES 5`, `FOND_DISTANCE 24`, `ROCHE_MIN 28`, `SEGMENT_MIN 6`, `PART {calcaire 1, granite 0.3, argile 0.1}`, `ESPACEMENT 120`, `PLANCHER_RAYON 100`, `TRACE_EAU 6`, `PAROI 1` (G-R12). Ce sont des ordres de grandeur : ils se règlent **en regardant une carte** (`tools/apercu-carte`), et la spec ne les tient pas pour vrais avant la mesure.
...
- **G-A15 — La paroi tient une tuile** (G-R12) : sur 4 graines, toute tuile creusée d'un karst — de la roche comme du plancher — hors des portes (la paire de gueule et les deux tuiles derrière) a ses huit voisines au palier ≥ `p+1` et dans la carte ; la garde a rougi sur le code d'avant (93 à 155 tuiles contre la falaise par graine, sur chaque karst) ; le compte de karsts par graine est rapporté avant/après. MESURÉ le 2026-09-20 (graines 2026/7/99/1234) : 0 tuile contre la falaise après, **aucun karst perdu** (8/17/11/12 avant comme après), les grottes perdent ~8 % de tuiles (1572→1444, 3430→3191, 2336→2023, 2625→2430), le plancher G-A7 inchangé (une Grotte du plancher de plus sur la graine 99).
