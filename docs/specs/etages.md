...
- **E-R5 — UN SEUL ACCESSEUR, jamais une condition recopiée.** La règle est : *deux points s'atteignent s'ils sont **au même étage**, **ou** si l'un est à moins de `N` tuiles d'un **connecteur** qui les relie.* Elle s'écrit une fois, dans un helper nommé, et les 67 sites l'appellent. Une seconde écriture de la même règle, c'est le loup qui mord à travers la roche parce qu'un escalier est à côté. **Précisé le 2026-09-20 (Alexis : *« on ne garde pas la porosité »*) — LA ROCHE EST ÉTANCHE** : la clause du connecteur ne vaut qu'entre deux étages du sol (la pente ouverte d'une rampe) ; un étage NÉGATIF (G-R1) ne se rejoint que depuis son propre étage, jamais par sa gueule ni à `N` tuiles d'elle — la gueule se FRANCHIT par le pas, elle ne s'atteint pas. MESURÉ avant : depuis la prairie à l'aplomb d'une salle, on visait la bête du vestibule, et elle sentait, chargeait et encornait le joueur à travers le plancher (pv 100 → 76 en 16 s) — la perception de la faune (`nearestThreat`) et le coup de charge du sanglier passent par E-R5 comme `resolveStrike`.
...
- **E-A6 — La carte générale ignore les étages** (contrainte 3) : `vignette.ts` et le champ de cendre rendent exactement la même image avec ou sans étages supérieurs. *(Amendée le 2026-09-20 par `flanc.md` F-R11 : quand la cendre montera par le bas, le champ de cendre de la RACINE jouée lira `palier` — E-A6 ne vaudra plus que pour `vignette.ts` et pour la vallée dormante. Tient encore : rien de codé côté cendre.)*
...
- **La torche est chaude parce qu'elle ôte du bleu** : MULTIPLY ambré (`CHALEUR_ALPHA`) puis ADD (`BRAISE_ALPHA`), même disque. Sous terre elle brûle à sa force de nuit ; l'ambiante Light2D de cave est `0x808898`. **Le jour au sol** est une nappe ADD chaude (`JOUR_SOL_ALPHA` 0,34), la moitié nord d'un disque posé au pied de la façade, qui s'éteint vers le nord ; **le dehors** se voit par la gueule comme une arche de jour dans la façade percée, la fente de dehors à sa forme et à sa place, teintée de l'heure (`grottes.md` §4sexies) — depuis le fond, c'est vers quoi on revient.
- **La gueule, dehors, est UNE image de trois rangées sur deux colonnes** (`dessinDeLaGueuleEntiere`, `GUEULE_LARGEUR` × 48 = 32×48), posée depuis la tuile OUEST de la paire, flancs à `tx − 1` et `tx + 2` : la fente s'évase vers le bas (24 px d'ouverture au seuil), lèvre claire à l'ouest, joue sombre à l'est, linteau d'ombre, sol de salle visible au bas de la fente, coulée d'humidité, seuil piétiné. Dedans, la façade percée et l'arche de jour (32×32, sur les rangées de la fente de dehors — `grottes.md` §4sexies), la nappe de jour (32×48, les trois rangées au-dessus du seuil) et le point de gueule se posent **une fois par paire**, au centre de la paire. Empilée en trois images, elle laissait passer une ligne de falaise à chaque couture au zoom 2,25 — **cause MESURÉE, et générale** : texture de puissance de deux → `gl.REPEAT`, et le MSAA extrapole l'UV des fragments de bord jusqu'à la rangée opposée de la texture ; remède `epinglerLaTuile` (`setVertexRoundMode('full')`) sur toute tuile de grille pleine — falaises, parois, pavés, vue instantanée.
