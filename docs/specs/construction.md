...
- **R24 — Rendu des toits.** Un toit occulte l'intérieur en vue de dessus → **fade/cutaway** quand l'avatar entre sous la couverture. Tâche client. *(Livré autrement, et mieux : la règle des **pans** — un côté de bâtiment tombe d'un bloc à ≤ 2 tuiles, découpe au sol plutôt que transparence. Voir `render/pans.ts` et la décision du 2026-07-27.)* *(Complété le 2026-08-10, calage instrumenté : le plan de toit se dessine **à la crête du mur** — levé de `MUR_HT`, couture mesurée à −2 px contre la façade sud — et l'effacement reste porté par la **nappe**, qui ne s'arme que si la tuile de départ est **sous un toit** : un dallage à ciel ouvert est dehors. Dedans, le toit **disparaît complètement** (alpha 0, décision d'Alexis — « comme les murs ») : pas de fantôme de chaume. Scénario de mesure : `pnpm smoke --scenario toits --dev` ; décisions du jour dans `docs/decisions.md`.)*
...
- **R27 — La PORTE DOUBLE se DÉRIVE, elle ne se pose pas** *(demande d'Alexis, 2026-08-01 : « un seul cadre, 2 battants se rejoignant au centre », constructible « en horizontal ou vertical » — l'entrée d'un village)*.
  - **Deux portes font une porte double.** Deux `door` sur des arêtes **colinéaires adjacentes** (la même ligne physique, quelle que soit l'adresse que chacune porte — R25, l'arête a deux adresses) forment une **paire**, si et seulement si la file colinéaire compte **exactement deux** portes (à trois, l'appariement serait ambigu : toutes restent simples). Pas de nouveau `StructureType`, pas d'entrée de menu, pas de migration : le joueur pose deux portes côte à côte et le cadre fusionne — et la **porte charretière** des villages PNJ (`village-plan.ts`, déjà deux vantaux) devient une porte double sans qu'une donnée ne bouge.
  - **Horizontal et vertical par la même règle** : bits N/S sur deux tuiles voisines en X, bits E/O sur deux tuiles voisines en Y.
  - **Un seul cadre au rendu.** Chaque vantail garde son sprite (une structure = un sprite), mais sa **famille** change : moitié `a` (la plus à l'ouest / au nord) et moitié `b`, chacune avec son **seul jambage extérieur** — au milieu, aucun montant, les battants se **rejoignent sur la limite de tuile**. Crête et linteau débordent comme les bandes de mur (R25) : les deux moitiés se recousent sans couture. Les battants s'ouvrent en **miroir**, tous deux vers le côté visible (le bas de l'écran / l'est), chacun se rangeant contre sa propre maçonnerie.
  - **Pousser un battant pousse les deux.** `toggle_door` applique l'état **résultant** du battant visé à son apparié (synchronisation, pas double bascule : deux battants désaccordés se réalignent) — **si** l'acteur a aussi accès à l'apparié, sinon seul le visé bouge. Un `door_toggled` par battant qui **change** réellement (un fait par fait). Le rituel de l'aube (villages PNJ) ouvrait déjà tous les vantaux d'un coup.
  - **Tout le reste est inchangé, et c'est le prix de la dérivation** : collision, PV, matériau, réparation, amélioration et démolition restent **par vantail** — un pillard qui casse un battant laisse l'autre debout (une demi-entrée, lisible). `blocksNavigation`, l'enceinte (R13) et les PNJ (`opensDoors`) ne voient aucune différence.
...
- **A25 — La porte double (R27)** : deux portes sur arêtes colinéaires adjacentes s'apparient **par les deux adresses** (l'une déclarée « (x,y)+S », l'autre « (x+1,y+1)+N » s'apparient quand même) ; une file de **trois** portes colinéaires n'apparie personne ; perpendiculaires ou espacées, pas de paire. Pousser un vantail ouvre les deux (**deux** `door_toggled`) ; les repousser referme les deux ; deux vantaux **désaccordés** se réalignent sur l'état résultant du visé (**un seul** événement, celui du battant qui change) ; sans accès à l'apparié, seul le visé bascule. Démolir un vantail rend l'autre **simple**. Rejeu au bit près. Client : les familles `st-door2a/door2b-e<bit>-f<n>` existent (éclairées comprises), le passage d'une moitié va du jambage extérieur à la **limite de tuile** — couvert à la frame 0, libre à la dernière, dégagé strictement à chaque pas — et ni `door2a` ni `door2b` ne figurent dans `COUPE_DE` (une porte reste debout).
